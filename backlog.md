...
**Приёмочная.** KOMPOT вынесен из одного приложения и заявлен как тулкит, не знающий предметной
области. Свидетель у этого утверждения был ровно один — то самое приложение. tacku — вторая,
независимая реализация протокола, и её задача — найти места, где протокол на самом деле знает про
предметную область происхождения или про Kotlin. Инструмент измерения — набор проверок `kompot-tck`
и [журнал вопросов](docs/research/questions.md); обоснования — в
[ресёрче](docs/research/research-architecture.md).

Цель сработала: 51 запись в журнале, 16 задач наверх, 8 изменений провода.
...
| [B-51](docs/backlog/B-51-two-screens-are-photographed-but-not-checked.md) `[ ]` | Вернуть в сверку два экрана, снятых, но не проверяемых | P2 | S | - |
