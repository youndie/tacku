...
## Ответ наверху: поле появилось, и тест, ждавший его, сработал

kompot 0.21 дал `finishLabel` у `wizard_screen`. Только у завершающей кнопки: «Назад» и «Далее»
перемещают, а не совершают, и цена их формулировки не та же.

**Обе проверки, написанные под отсутствие поля, покраснели в день его появления** — они и писались
так, чтобы это случилось: одна на Go по профилю, одна на Kotlin по дескриптору опубликованного типа.
Обе заменены проверками той же формы: поле обязано быть, и слово в шапке обязано быть ровно одно —
слово, которого сервер не заполняет, выбрал кто-то другой.

Сервер теперь называет кнопку по тому, что она делает: «Create the task», а не «Finish», — потому
что «Finish» верно для любого сценария и не говорит человеку ничего о том, что сейчас произойдёт.
