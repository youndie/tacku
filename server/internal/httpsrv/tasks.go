...
func myTasks(store domain.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// The rail, because the alternative is a screen you can only leave by quitting. "My tasks"
		...
		screen := render.Row("screen-my-tasks-frame", 0,
			[]render.Modifier{render.FillWidth(), render.FillHeight(), render.Background(render.ColorSurface)},
			render.Navigation(principal.Provenance.OnBehalfOf, render.LinkMyTasks),
			render.Rule("my-tasks-nav-rule", render.RuleDp, render.ColorDivider, false),
			body,
		)
