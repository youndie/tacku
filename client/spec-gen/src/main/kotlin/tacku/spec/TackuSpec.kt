...
object TackuSpec {
     ...
     *
     * `multiline_input` was here for one release and is gone. It existed because the cheap shape of
     * the same addition — an optional flag on a type the toolkit owns — was unavailable to a
     * deployment, and asking for it is what removed the need: kompot 0.21 carries `multiline` on
     * `text_input`. A wire type of ours saying what the vocabulary now says would be a second way to
     * say one thing, and the expensive one: an unfamiliar component costs a placeholder, an
     * unfamiliar flag costs nothing.
     */
